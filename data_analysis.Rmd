...
s_basolo %>%
  left_join(adiv_basolo, by = "sample_id") %>%
  left_join(idxdf_basolo, by = "sample_id") %>%
  cor.test(~ vanco_idx + shannon, method = "spearman", data =.)
```
...
dysbiosis_cor_sprocket <- s_sprockett %>%
  ...
  nest_by(study_day) %>%
  summarise(
    tidy(cor.test(~ dysbiosis + metro_idx, method = "spearman", data = data)),
    .groups = "drop") %>%
  mutate(sig = cut(
...
s_sprockett %>%
  ...
  geom_text(aes(label = str_glue(
    "rho == {round(estimate, 2)}~\"{sig}\"")),
    data = dysbiosis_cor_sprocket,
    x = 1.7, y = 0.5, parse = TRUE) +
...
props_hmap_boynton %>%
  pheat() %>%
  pheat_display_cols(gaps = factor_gaps(paste(s_boynton$study_day, s_boynton$study_group))) %>%
...
idxdf_boynton %>%
  left_join(s_boynton, by = "sample_id") %>%
  lm(doxy_idx ~ study_group * study_day, data = .) %>%
  summary()
```
...
s_boynton %>%
  left_join(adiv_boynton, by = "sample_id") %>%
  lmer(shannon ~ study_group * study_day + (1|subject_id), data = .) %>%
  summary()
```
...
dysbiosis_boynton %>%
  left_join(s_boynton, by = "sample_id") %>%
  lmer(dysbiosis ~ study_group * study_day + (1|subject_id), data = .) %>%
  summary()
```
...
diagram_samples <- tibble(
  sample_id = paste("Sample", LETTERS[1:4]),
  resistant_prop = c(0.2, 0.4, 0.52, 0.9)) %>%
  mutate(mirix_val = log10(resistant_prop / (1 - resistant_prop)))
diagram_taxa <- tibble(
...
diagram_samples %>%
  ...
  mutate(susceptibility = fct_rev(susceptibility)) %>%
